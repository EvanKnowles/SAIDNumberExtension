...
- The Chrome Web Store `description` field has a hard 132-character limit — past commits had to
  trim copy to fit it.
- `jQuery.js` is a vendored jQuery 2.0.2. The validity indicator is pure CSS (a coloured badge
  with a `\2713` / `\2715` glyph via `.id-status.valid` / `.id-status.invalid`) — the old
  `Circle_Green.png` / `Circle_Red.png` images were removed.
...
2. **Popup** (`generator.html` → `jQuery.js` + `jSAID.js` + `idgenerator.js`)
   Thirteen single-char `.id-omnibox` inputs act as one ID field (per-digit keydown handling,
   arrow/backspace navigation) — they must stay **direct adjacent siblings** inside
   `.id-text-container` for `.prev()/.next()` and the `:nth-child` group gaps to work.
   `idgenerator.js` wires the Random / Generate / Copy buttons, drives the `.id-status` badge via
   `extractFromID`, and mirrors the parsed birthdate / gender / citizenship into the controls.
   The Male / SA-citizen toggles are `<input type="checkbox" class="male|citizen">` styled as
   sliders — the checkbox stays the source of truth (`.is(':checked')` / `.prop('checked', …)`).
   The last copied (or last persisted) ID is saved to `localStorage["last-id"]` and restored on
   open; Generate clears it by storing the string `"null"`. `idgenerator.js` uses only class
   selectors, so markup can be restyled freely as long as the classes stay.
...
- `idgenerator.js` (popup only) still uses `localStorage` and `document.execCommand('copy')` —
  both are fine in an MV3 extension **page**, but neither exists in the service worker, so don't
  move that logic into `main.js`.
- jQuery 2.0.2 is long past end-of-life but loads fine under the MV3 CSP (local file, no
  `eval`/remote code on the paths the popup uses). It is popup-only; never load it in the worker.
